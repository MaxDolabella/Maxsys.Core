...
public interface IModelService<TEntity, TKey> : IModelService<TEntity>
    where TEntity : class
    where TKey : notnull
{
    #region EVENTS
    ...
    /// <summary>
    /// Retorna uma lista de <see cref="InfoDTO{TKey}"/> aplicando <paramref name="modelFilters"/> em <typeparamref name="TEntity"/>
    /// e <paramref name="criteria"/> (filtros de DTO, ordenação e paginação) após a projeção.
    /// </summary>
    /// <remarks>Mapeamento necessário: <typeparamref name="TEntity"/> → <see cref="InfoDTO{TKey}"/> </remarks>
    Task<List<InfoDTO<TKey>>> ToInfoListAsync(ICollection<ColumnFilter> modelFilters, ListCriteria criteria, CancellationToken cancellationToken = default);

    /// <remarks>Mapeamento necessário: <typeparamref name="TEntity"/> → <see cref="InfoDTO{TKey}"/> </remarks>
    Task<List<InfoDTO<TKey>>> ToInfoListAsync(ListCriteria criteria, CancellationToken cancellationToken = default);

    /// <remarks>Mapeamento necessário: <typeparamref name="TEntity"/> → <see cref="InfoDTO{TKey}"/> </remarks>
    Task<List<InfoDTO<TKey>>> ToInfoListAsync(ICollection<ColumnFilter> filters, Pagination? pagination, Expression<Func<InfoDTO<TKey>, dynamic>> sortSelector, SortDirection sortDirection = SortDirection.Ascending, CancellationToken cancellationToken = default);

    /// <remarks>Mapeamento necessário: <typeparamref name="TEntity"/> → <see cref="InfoDTO{TKey}"/> </remarks>
    Task<List<InfoDTO<TKey>>> ToInfoListAsync(ICollection<ColumnFilter> filters, CancellationToken cancellationToken = default);
    ...
    /// <remarks>Mapeamento necessário: <typeparamref name="TEntity"/> → <see cref="InfoDTO{TKey}"/> </remarks>
    Task<ListDTO<InfoDTO<TKey>>> GetInfoListAsync(ListCriteria criteria, CancellationToken cancellationToken = default);

    /// <summary>
    /// Retorna um <see cref="ListDTO{T}"/> de <see cref="InfoDTO{TKey}"/> aplicando <paramref name="filters"/> no dto (<see cref="InfoDTO{TKey}"/>>).
    /// </summary>
    /// <remarks>Mapeamento necessário: <typeparamref name="TEntity"/> → <see cref="InfoDTO{TKey}"/> </remarks>
    Task<ListDTO<InfoDTO<TKey>>> GetInfoListAsync(ICollection<ColumnFilter> filters, CancellationToken cancellationToken = default);

    /// <summary>
    /// Retorna um <see cref="ListDTO{T}"/> de <see cref="InfoDTO{TKey}"/> aplicando <paramref name="modelFilters"/> em <typeparamref name="TEntity"/>
    /// e <paramref name="criteria"/> (filtros de DTO, ordenação e paginação) após a projeção.
    /// </summary>
    /// <remarks>Mapeamento necessário: <typeparamref name="TEntity"/> → <see cref="InfoDTO{TKey}"/> </remarks>
    Task<ListDTO<InfoDTO<TKey>>> GetInfoListAsync(ICollection<ColumnFilter> modelFilters, ListCriteria criteria, CancellationToken cancellationToken = default);
