...
public abstract class HttpClientBase : ServiceBase
{
    ...
    {
        // UnauthorizedAccessException (no content)
        if (response.Message.StatusCode == System.Net.HttpStatusCode.Unauthorized)
        {
        ...
        {
            _logger.LogError("ExternalAPIException: No Content Response");
            throw new ExternalAPIException(response.Message.StatusCode, "No Content Response");
        }
        ...
        {
            throw new ExternalAPIException(ex, response.Message.StatusCode, response.Content);
        }
    }
